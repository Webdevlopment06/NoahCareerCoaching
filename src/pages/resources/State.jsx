...
export default function State() {
  ...
  const labels = useMemo(
    () => STATES.map((s) => ({ id: `label-${s.abbr}`, ...s })),
    [],
  );
  ...
  useEffect(() => {
    ...
    const collapseEl = container
      ? container.querySelector(`#${collapseId}`)
      : document.getElementById(collapseId);
    const headerEl = container
      ? container.querySelector(`#${headerId}`)
    ...
    setTimeout(() => {
      (headerEl || collapseEl)?.scrollIntoView({
        behavior: "smooth",
        block: "center",
      });
    }, 150);
        ...
        <div className="row">
          <div className="col-md-7">
            <div id="map-container">
              <img
                src="/images/resources-imgs/statemap.png"
                alt="USA Map"
                className="img-fluid"
                loading="lazy"
                decoding="async"
          ...
          <div className="col-md-5">
            <div id="state-info-box" className="p-3 border rounded">
              ...
              <p className="fw-bold fs-5 text-primary" id="state-name-display">
                {selected
                  ? `${STATES.find((s) => s.abbr === selected).name} (${selected.toUpperCase()})`
                ...
                {selected ? (
                  <>
                    <p>
                      ...
                      {STATES.find((s) => s.abbr === selected).name}.
                    </p>
                    <Link
                      className="btn btn-sm btn-outline-primary"
                      to={`/resources/state/${selected}`}
                    >
                      Go to {selected.toUpperCase()} page
                    </Link>
                  </>
                ) : (
                  <p>
                    At Noah Career Coaching, we provide local job boards,
                    training programs, workforce centers, and community services
                    available where you live. Click a state to access resources.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="accordion-item" key={s.abbr}>
              <h2 className="accordion-header" id={`heading-${s.abbr}`}>
                <button
                  ...
                  data-bs-target={`#collapse-${s.abbr}`}
                  aria-expanded="false"
                  aria-controls={`collapse-${s.abbr}`}
                >
              ...
              <div
                ...
                aria-labelledby={`heading-${s.abbr}`}
                data-bs-parent="#accordionFlushExample"
              >
                <div className="accordion-body">
                  <StateContent abbr={s.abbr} />
                </div>
              </div>
            </div>
          ))}
